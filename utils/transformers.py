...
class Conv1d_BN(nn.Sequential):
    def __init__(self, a, b, bias=True,ks=1, stride=1, pad=0, dilation=1,
...
class trainablePowerSoftmax(nn.Module): # trainableQuad2 w/o mask
    def __init__(self, attn, power, input_N_dim=None, S=None, NS=None, num_heads=None, dim=-1, usc_c=False, use_a=False,  use_d=False, use_e=False, init=False):
        super(trainablePowerSoftmax, self).__init__()
        self.dim=dim
        #self.c=c
...
class originalheadlevelnorm(nn.Module): # quad2 w/o mask
    def __init__(self, c=5, dim=-1):
        super(originalheadlevelnorm, self).__init__()
        self.dim=dim
        self.c=c
    ...
    def forward(self, x, attention_mask_quad=None):
        ...
        #x=x*attention_mask_quad
        #print (f"before sum: {x.shape}")
        #s=torch.sum(x, dim=self.dim, keepdim=True)
        s = torch.sum(torch.sum(x, dim=-1, keepdim=True), dim=-2, keepdim=True)
        #print (f"after sum: {s.shape}")
        #print (s)
        ...
        output=x/ s
        #print (torch.sum(torch.sum(output, dim=-1), dim=-1))
...
class originallayerlevelnorm(nn.Module): # quad2 w/o mask
    def __init__(self, c=5, dim=-1):
        super(originallayerlevelnorm, self).__init__()
        self.dim=dim
        self.c=c
    ...
    def forward(self, x, attention_mask_quad=None):
        ...
        #print (f"before sum: {x.shape}")
        #s=torch.sum(x, dim=self.dim, keepdim=True)
        s = torch.sum(torch.sum(torch.sum(x, dim=-1, keepdim=True), dim=-2, keepdim=True), dim=-3, keepdim=True)
        #print (f"after sum: {s.shape}")
        #print (s)
        ...
        output=x/ s
        #print (torch.sum(torch.sum(torch.sum(output, dim=-1), dim=-1), dim=-1))
...
class MPC_ExternalAttention(Module):
    ...
    def __init__(self, dim, num_heads=8, MPC_externalattention_dim=128, externalattention_dim=64, input_N_dim=256, attention_dropout=0.1, projection_dropout=0.1, softmax_approx='original', externalattention_divhead=False, allargs=None):
        ...
        self.CS = allargs.externalattention_dim
        #self.NS=MPC_externalattention_dim
        self.NS = allargs.MPC_externalattention_dim
        ...
        elif softmax_approx=='powersoftmax':
            self.softmaxlayer=trainablePowerSoftmax(attn='mpcext',
                                               power=allargs.powersoftmax,
                                               S=self.CS,
                                               NS=self.NS,
                                               dim=-1,
                                               )
...
class Attention(Module):
    ...
    def __init__(self, dim, num_heads=8, attention_dropout=0.1, projection_dropout=0.1, softmax_approx='original', learnable_c=None,learnable_d=None, allargs=None,):
        ...
        if softmax_approx=='original':
        ...
        elif softmax_approx=='originalheadlevelnorm':
            self.softmaxlayer=originalheadlevelnorm()

        elif softmax_approx=='originallayerlevelnorm':
            self.softmaxlayer=originallayerlevelnorm()
            ...
            self.softmaxlayer = trainablePowerSoftmax(attn='self',
                                                power=allargs.powersoftmax,
                                                 input_N_dim=allargs.input_N_dim,
                                                 num_heads=self.num_heads,
                                                 dim=-1,
...
class separableAttention(Module):
    """
    Obtained from https://github.com/apple/ml-cvnets/blob/6acab5e446357cc25842a90e0a109d5aeeda002f/cvnets/layers/linear_attention.py#L51
    """
    def __init__(self, dim, num_heads=8, attention_dropout=0.1, projection_dropout=0.1, softmax_approx='original',
                 learnable_c=None, learnable_d=None, allargs=None, ):
        super().__init__()
        print (f"separableAttention softmax:{softmax_approx}")

        self.dim=dim
        self.num_heads = num_heads
        head_dim = dim // self.num_heads
        self.scale = head_dim ** -0.5
        self.useRelu=allargs.useRelu
        self.separablelayer=allargs.separablelayer

        if allargs.separablelayer=='linear':
            self.qkv = Linear(dim, 1 + dim * 2, bias=False)

            self.out_proj = Linear(dim, dim)

        elif allargs.separablelayer=='conv':
            self.qkv = Conv1d_BN(dim, 1 + dim * 2, 1)
            self.out_proj = Conv1d_BN(dim, dim, 1)
    ...
    def forward(self, x):
        ...
        #print (f"qkv.shape: {qkv.shape}")
        q, k, v = torch.split(
            qkv, split_size_or_sections=[1, self.dim, self.dim], dim=2
        )
        ...
        context_scores =self.softmaxlayer(q)
        ...
        #print (k[0][:,0])
        #print (context_scores[0])
        # [B, N, C] x [B, N, 1] => [B, N, C]
        context_vector = k * context_scores
        #print (context_vector.shape)
        #print (context_vector[0][:, 0])
        ...
        #print (context_vector.expand_as(v))
        #print (context_vector.expand_as(v).shape)
        # [B, N, C] x [B, 1, C] => [B, N, C]
        out=v * context_vector.expand_as(v)
...
class SA2Linear(Module):
    ...
    def __init__(self, dim, num_heads=8, attention_dropout=0.1, projection_dropout=0.1, softmax_approx='original', learnable_c=None,learnable_d=None, allargs=None,):
        ...
        self.input_N_dim = allargs.input_N_dim
        self.S=allargs.externalattention_dim
        print (f"attention softmax:{softmax_approx}, {self.S}")
        self.Mq = Linear(self.input_N_dim, self.S, bias=False)
        self.Mk = Linear(self.input_N_dim, self.S, bias=False)
        self.NMq = Linear(self.S, self.input_N_dim, bias=False)
        self.NMk = Linear(self.S, self.input_N_dim, bias=False)
    ...
    def forward(self, x):
        ...
        #print (f"after k.shape:{k.shape}")

        attn = (q @ k.transpose(-2, -1)) * self.scale
        #print (f"after k.shape:{attn.shape}")
        #attn = (q.transpose(-2, -1) @ k) * self.scale

        #print (f"before Mq.shape:{attn.shape}")
        attn=self.Mq(attn)
        #print (f"after Mq.shape:{attn.shape}")
        ...
        #print (f"after softmax, B, num_head, NS, NS: {attn.shape}")

        #attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        attn=self.NMq(attn)
        #print (f"after NMq: {attn.shape}")
        attn=attn.permute(0, 1, 3, 2)
        #print (f"after permute: {attn.shape}")

        attn=self.NMk(attn)
        #print (f"after NMv: {attn.shape}")
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        if attention_mechanism=='original':
            ...
            self.self_attn = ExternalAttention(dim=d_model,  num_heads=allargs.num_heads, externalattention_dim=externalattention_dim, externalattention_divhead=externalattention_divhead,
                                       attention_dropout=attention_dropout, projection_dropout=dropout, softmax_approx=softmax_approx, allargs=allargs)
        ...
        elif attention_mechanism=='sa1linear':
            self.self_attn = SA1Linear(dim=d_model, num_heads=nhead,
                                       attention_dropout=attention_dropout, projection_dropout=dropout,
                                       softmax_approx=softmax_approx, allargs=allargs)

        elif attention_mechanism=='sa2linear':
            self.self_attn = SA2Linear(dim=d_model, num_heads=nhead,
                                       attention_dropout=attention_dropout, projection_dropout=dropout,
                                       softmax_approx=softmax_approx, allargs=allargs)

        elif attention_mechanism=='separableAttention':
            self.self_attn = separableAttention(dim=d_model, num_heads=nhead,
                                       attention_dropout=attention_dropout, projection_dropout=dropout,
