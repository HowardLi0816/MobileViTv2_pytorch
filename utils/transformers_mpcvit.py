...
class Attention(Module):
    ...
    def __init__(self, dim, num_heads=8, attention_dropout=0.1, projection_dropout=0.1, softmax_approx='original', learnable_c=None,learnable_d=None, allargs=None,):
        ...
        elif softmax_approx=='mpcvit':
            self.alpha=nn.Parameter(torch.zeros(1, self.num_heads, 1, 1), requires_grad=True)
            self.relu2=ReluSoftmax()
            self.scaleattn=ScaleAttn(self.scale)
    ...
    def forward(self, x):
        ...
        #print (f"before softmax: {attn.shape}")
        if softmax_approx!='mpcvit':# normal softmaxlayer or approximation
            attn=self.softmaxlayer(attn)
        else: # mpcvit
            attn=self.alpha*self.relu2+self.alpha*self.scaleattn
